...
> ![]()
``` bash
$ python aiparkhub/core/models/faces_positioning/find_faces_in_batches.py --cpus -1
```
...
#### 4.1.3 人像面部识别
- [示例: 人脸识别之后在原图上画框框并标注姓名](aiparkhub/core/models/faces_recognition/identify_and_draw_boxes_on_faces.py)
> ![]()
``` bash
$ python  --cpus 4
```

- [示例: 在不同精度上比较两个人脸是否属于一个人](aiparkhub/core/models/faces_recognition/face_distance.py)
> ![]()
``` bash
$ python  --cpus 4
```

- [示例: 人脸识别-较慢版](aiparkhub/core/models/faces_recognition/feature_recognition.py)
> ![]()
``` bash
$ python  --cpus 4
```

- [示例: 进行人脸个数统计及人脸身份识别](aiparkhub/core/models/training_feature/training_portrait_feature_models.py)
> ![]()
``` bash
$ python  --cpus 4
```

- [示例: 基于K最近邻KNN分类算法进行人脸识别](aiparkhub/core/models/faces_recognition/face_recognition_knn.py)
> ![]()
``` bash
$ python  --cpus 4
```
